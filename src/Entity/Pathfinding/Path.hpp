...
struct Path
{
    std::vector<glm::vec3> look_points;
    size_t finish_line_index;
    size_t slow_down_index;

    Path(const std::vector<glm::vec3>& waypoints, float stopping_dst)
    {
        ...
        {
            dst_from_end_point += glm::distance(look_points[i], look_points[i - 1]);
